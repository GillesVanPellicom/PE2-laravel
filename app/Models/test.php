...
class test extends Model {
  ...
  public function move(MoveOperationType $operation): array {
    ...
    $currentMovement = $this->movements()->where("current_node_id",
      $this->current_location_id)->first(); // Find the corresponding package movement
    if (!$currentMovement) {
    ...
    if ($operation == MoveOperationType::DELIVER) {
      ...
      if (!is_numeric($this->current_location_id) || Location::find($this->current_location_id)->location_type != NodeType::ADDRESS) {
        return [
          false,
          "This package is not fit for delivery =>".Location::find($this->current_location_id)->location_type->value
        ];
      }
      ...
      if (is_null($currentMovement->arrival_time)) {
        ...
        if (is_null($currentMovement->check_in_time)) {
          $currentMovement->check_in_time = now();
          $appliedMode = MoveOperationType::IN;

        } else {
          if (is_null($currentMovement->check_out_time)) {
            $currentMovement->check_out_time = now();
            $appliedMode = MoveOperationType::OUT;

          } else {
            if (is_null($currentMovement->departure_time)) {
