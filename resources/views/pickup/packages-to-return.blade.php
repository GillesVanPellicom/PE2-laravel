<x-app-layout>
    @section('title', 'Packages to return')
    <x-slot name="header">
        <div class="flex flex-row max-w-2xl items-center justify-center">
            <!-- Back Button -->

            <form action="{{ url()->previous() === url()->current()."?" ? route('pickup.dashboard') : url()->previous() }}" method="get">
                <x-primary-button>
        ...
        <table class="min-w-full table-auto bg-gray-200 shadow-xl  sm:rounded-b-lg overflow-hidden">
            <div class=" bg-gray-50 overflow-hidden shadow-lg sm:rounded-t-lg flex items-center sm:justify-between p-3">
                ...
                <div class="flex justify-between items-center gap-3">
                    <form action="{{route('pickup.dashboard.packages-to-return')}}" method="get">
            ...
            <tr>
                ...
                <th class="py-3 px-4 text-center">Arrived on</th>
                <th class="py-3 px-4 text-center">Expiry date on </th>
                <th class="py-3 px-4 text-center">Actions</th>
            </tr>
            </thead>
            <tbody>
            @if(count($packagesThatNeedToBeReturned) == 0)
                        <tr class="border-b last:border-b-0 text-center hover:bg-gray-50 bg-gray-50 ">
                            <td class="py-3 px-4" colspan="6">No expired Packages for today :)</td>
                        </tr>
            @else
                        @foreach($packagesThatNeedToBeReturned as $package)
                        ...
                        <td class="py-3 px-4">{{ \Carbon\Carbon::parse($package->updated_at)->addDay(7)->format('Y-m-d H:i') }}</td>
                ...
                <div class="min-w-full flex justify-center items-center gap-6 mt-4">
                    {{$packagesThatNeedToBeReturned->onEachSide(1)->links()}}
                    </div>
                <div>
